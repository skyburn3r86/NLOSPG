...
%GEOMETRY Generates Geometry of Initialized Parameters (Setup)
	...
	geom_dummy.set('base','center');
    geom_dummy.set('pos', {'0' '-hWG_photonic/2'});
    geom_dummy.set('size', {'wWG' 'hWG_photonic'});    
    model.component('comp1').geom('geom1').feature('r_photonic_wg_bot').set('contributeto', materialNames(2));

    % generating top WG 
    geom_dummy = model.component('comp1').geom('geom1').create(['r_' 'plasmonic_wg_top'], 'Rectangle');
    geom_dummy.label('photonic_wg_top');
	geom_dummy.set('base','center');
    geom_dummy.set('pos', {'0' 'hWG_metal/2 + hOEO'});
    geom_dummy.set('size', {'wWG' 'hWG_metal'});    
    model.component('comp1').geom('geom1').feature('r_plasmonic_wg_top').set('contributeto', materialNames(5));
